...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    delta_k = K @ (y_k - p_check)

    # 3.3 Correct predicted state
    p_hat = p_check + delta_k[:3]
    v_hat = v_check + delta_k[3:6]
    q_hat = Quaternion(euler=delta_k[6:]).quat_mult_right(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k] = p_est[k-1] + delta_t * v_est[k-1] + 0.5 * delta_t ** 2 * (Cns @ imu_f.data[k-1] + g)
    ...
    F[3:6, 6:] = -skew_symmetric(Cns @ imu_f.data[k-1]) * delta_t
    Q = np.eye(6)
