...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            if a_desired > 0:
                throttle_output = np.tanh(a_desired)
            else:
                throttle_output = 0
            ...
            # if heading error greater than 180 degrees, then turn the other way
            ...
            yaw_path_to_crosstrack = yaw_path - yaw_cross_track
            # if yaw_path_to_crosstrack greater than 180 degrees, then turn the other way
            if yaw_path_to_crosstrack > np.pi:
                yaw_path_to_crosstrack -= 2 * np.pi
            if yaw_path_to_crosstrack < -np.pi:
                yaw_path_to_crosstrack += 2 * np.pi
            # sign of crosstrack error
            if yaw_path_to_crosstrack > 0:
                crosstrack_error = abs(crosstrack_error)
            else:
                crosstrack_error = -abs(crosstrack_error)
            # yaw cross track
            yaw_cross_track = np.arctan(k_e * crosstrack_error / v)

            # steering angle
            steer_angle = yaw_diff_heading + yaw_cross_track
            # if steering angle greater than 180 degrees, then turn the other way
